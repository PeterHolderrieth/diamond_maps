"""FID preprocessing, InceptionV3, and Frechet distance utilities."""
...
def resize_and_incept(imgs32: jnp.ndarray, inception: Callable) -> jnp.ndarray:
    imgs299 = jax.image.resize(
        imgs32,  # NHWC -> (B,299,299,3)
        (imgs32.shape[0], 299, 299, 3),
        method="bilinear",
    )
    return inception(imgs299)  # returns (B,2048)


def process_celeba_for_fid(ex: Dict) -> tf.Tensor:
    """Crop an image to 140x140, then resize to 64x64 pixels."""
    image = tf.cast(ex["image"], tf.float32) / 127.5 - 1.0  # (32,32,3), [-1,1]
    crop = tf.image.resize_with_crop_or_pad(image, 140, 140)
    crop64 = tf.image.resize(crop, [64, 64], method="area", antialias=True)
    return crop64


def process_image_for_fid(ex: Dict) -> tf.Tensor:
    img = tf.cast(ex["image"], tf.float32) / 127.5 - 1.0  # (32,32,3), [-1,1]
    return img
...
def download(url, ckpt_dir=None):
    ...
    if ckpt_dir is None:
        ckpt_dir = repo_paths.metric_models_path()
    ...
    if not os.path.exists(ckpt_file):
        ...
        # Use a process-local temp name so parallel metric jobs can race safely.
        ckpt_file_temp = os.path.join(ckpt_dir, f"{name}.{os.getpid()}.temp")
        ...
        if total_size_in_bytes != 0 and progress_bar.n != total_size_in_bytes:
            ...
            os.replace(ckpt_file_temp, ckpt_file)
    return ckpt_file
...
class InceptionV3(nn.Module):
    ...
    def __call__(self, x, train=True, rng=None):
        ...
        Args:
            ...
            rng (jax.random.PRNGKey): Random seed (only used if train=True for dropout).
        """

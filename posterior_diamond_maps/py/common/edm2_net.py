"""EDM2 UNet components and preconditioned flow-map wrappers."""
...
class EDM2FlowMapUNet(nn.Module):
    ...
    diamond_map: bool = False  # Whether to include diamond map conditioning
    ...
    def setup(self):
        ...
        self.emb_t_prime_fourier = MPPositionalEmbedding(cst)
        self.emb_t_linear = MPConv(cst, cemb)
        self.emb_t_prime_linear = MPConv(cst, cemb)

        if self.diamond_map:
            self.emb_s_fourier = MPPositionalEmbedding(cst)
            self.emb_s_prime_fourier = MPPositionalEmbedding(cst)
            self.emb_s_linear = MPConv(cst, cemb)
            self.emb_s_prime_linear = MPConv(cst, cemb)
        ...
        # Start with image channels + 1 (constant).
        # If diamond map, add image channels for x_t.
        cout = self.img_channels + 1
        if self.diamond_map:
            cout += self.img_channels
    ...
    def __call__(
        ...
        # process embeddingos
        emb_t = self.emb_t_linear(self.emb_t_fourier(t))
        emb_t_prime = self.emb_t_prime_linear(self.emb_t_prime_fourier(t_prime))
        emb = mp_sum(emb_t, emb_t_prime, t=0.5)

        if self.diamond_map:
            emb_s = self.emb_s_linear(self.emb_s_fourier(s))
            emb_s_prime = self.emb_s_prime_linear(self.emb_s_prime_fourier(s_prime))
            emb_diamond = mp_sum(emb_s, emb_s_prime, t=0.5)
            emb = mp_sum(emb, emb_diamond, t=0.5)
        ...
        # Base input concatenation: x + constant channel (bias)
        inputs = [x, jnp.ones_like(x[:, :1])]

        # If diamond map, concatenate source image x_t.
        if self.diamond_map:
            inputs.append(x_t)
...
class PrecondFlowMap(nn.Module):
    ...
    is_diamond: bool = False  # Whether to use diamond map logic
    ...
    def calc_weight(
        ...
        emb_t = self.logvar_fourier_t(t).reshape(-1, self.logvar_channels)
        emb_t_prime = self.logvar_fourier_t_prime(t_prime).reshape(-1, self.logvar_channels)
        emb = mp_sum(emb_t, emb_t_prime, t=0.5)

        if self.is_diamond:
            emb_s = self.logvar_fourier_s(s).reshape(-1, self.logvar_channels)
            emb_s_prime = self.logvar_fourier_s_prime(s_prime).reshape(-1, self.logvar_channels)
            emb_diamond = mp_sum(emb_s, emb_s_prime, t=0.5)
            emb = mp_sum(emb, emb_diamond, t=0.5)

        logvar = self.logvar_linear(emb).reshape(-1, 1, 1, 1)
        return logvar
    ...
    def process_input(
        ...
        """Process inputs for the model using t_dt parameterization."""
        t = t.astype(jnp.float32).reshape(-1, 1, 1, 1)
        t_prime = t_prime.astype(jnp.float32).reshape(-1, 1, 1, 1)
        x = x.astype(jnp.float32)

        # Transform from (t, t_prime) to (t, dt) where dt = t_prime - t.
        t_prime = t_prime - t

        if self.is_diamond:
            s = s.astype(jnp.float32).reshape(-1, 1, 1, 1)
            s_prime = s_prime.astype(jnp.float32).reshape(-1, 1, 1, 1)
            s_prime = s_prime - s
            x_t = x_t.astype(jnp.float32)
        return t, t_prime, x, s, s_prime, x_t
    ...
    def calc_phi(
        ...
        t, t_prime, x, s, s_prime, x_t = self.process_input(t, t_prime, x, s, s_prime, x_t)
        ...
        # Run the model
        x_in = (c_in * x).astype(dtype)

        # Handle x_t scaling if present
        x_t_in = (c_in * x_t).astype(dtype) if x_t is not None else None
        ...
        if init_weights:
            ...
            _ = self.calc_weight(t, t_prime, s, s_prime)

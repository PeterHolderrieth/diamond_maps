    ...
    def __call__(
        ...
        def save_latent_frame(latents_tensor: torch.Tensor, out_file: str) -> None:
            unpacked = self._unpack_latents(
                latents_tensor.to(self.vae.dtype),
                height,
                width,
        ...
        if save_intermediate_imgs:
            ...
            if save_path is None:
                save_path = f"flux_baseline/{prompt_slug(prompt_text)}"
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == len(timesteps) - 1:
                    ...
                    timestep2 = torch.zeros_like(
                        timestep
                    )  # TODO: check that 0 is data: https://arxiv.org/pdf/2506.15742
                else:
                    timestep2 = (
                        timesteps[i + 1].expand(latents.shape[0]).to(latents.dtype)
                    )
                ...
                if do_true_cfg:
                    ...
                    neg_noise_pred = self.transformer(
                        ...
                        timestep=torch.stack(
                            [timestep / 1000, timestep2 / 1000], dim=-1
                        ),
                        guidance=guidance,
                ...
                if save_intermediate_imgs:
                    x_t = latents.detach()
                    zero_timestep2 = torch.zeros_like(timestep2)
                    cond_joint_attention_kwargs = (
                        self.joint_attention_kwargs.copy()
                        if self.joint_attention_kwargs is not None
                        else {}
                    )
                    if image_embeds is not None:
                        cond_joint_attention_kwargs["ip_adapter_image_embeds"] = (
                            image_embeds
                        )
                    v0_pred = self.transformer(
                        hidden_states=latents,
                        timestep=torch.stack(
                            [timestep / 1000, zero_timestep2 / 1000], dim=-1
                        ),
                        guidance=guidance,
                        pooled_projections=pooled_prompt_embeds,
                        encoder_hidden_states=prompt_embeds,
                        txt_ids=text_ids,
                        img_ids=latent_image_ids,
                        joint_attention_kwargs=cond_joint_attention_kwargs,
                        return_dict=False,
                    )[0]
                    if do_true_cfg:
                        uncond_joint_attention_kwargs = (
                            self.joint_attention_kwargs.copy()
                            if self.joint_attention_kwargs is not None
                            else {}
                        )
                        if negative_image_embeds is not None:
                            uncond_joint_attention_kwargs["ip_adapter_image_embeds"] = (
                                negative_image_embeds
                            )
                        neg_v0_pred = self.transformer(
                            hidden_states=latents,
                            timestep=torch.stack(
                                [timestep / 1000, zero_timestep2 / 1000], dim=-1
                            ),
                            ...
                            txt_ids=negative_text_ids,
                            img_ids=latent_image_ids,
                            joint_attention_kwargs=uncond_joint_attention_kwargs,
                            return_dict=False,
                        )[0]
                        v0_pred = neg_v0_pred + true_cfg_scale * (v0_pred - neg_v0_pred)
                    sigma_t = t / 1000
                    x0_hat = latents - sigma_t * v0_pred
                    save_latent_frame(x_t, f"{xt_dir}/step_{i:03d}.png")
                    save_latent_frame(x0_hat, f"{x0_dir}/step_{i:03d}.png")

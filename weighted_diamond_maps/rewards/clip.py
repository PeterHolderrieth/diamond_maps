...
class CLIPLoss(BaseRewardLoss):
    ...
    def get_image_features(self, image: torch.Tensor) -> torch.Tensor:
        clip_img_features = self.clip_model.get_image_features(image).to(image.dtype)
    ...
    def compute_loss(
        ...
        clip_loss = 100 - (image_features * text_features).sum(
            dim=1
        ) * self.clip_model.logit_scale.exp().to(image_features.dtype)
